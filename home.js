...
homeRouter.get("/testing", (req, res) => {
    User.findOne({ username: "jimmyjohnson" }, (err, user) => {
        user.matches = [];
        user.messages = [];
        user.profilePic = "https://www.learning.uclg.org/sites/default/files/styles/featured_home_left/public/no-user-image-square.jpg?itok=PANMBJF-";
        user.save();
        res.send(user);
    })
})
...
function getGender() {
    let randomGender = randomInt(1, 13);
    if (randomGender == 1) {
        return "Other";
    } else if (randomGender > 1 && randomGender <= 7) {
        return "Male";
    } else {
        return "Female";
    }
...
function getLookingFor() {
    let randomLookingFor = randomInt(1, 13);
    if (randomLookingFor == 1) {
        return "Both";
    } else if (randomLookingFor > 1 && randomLookingFor <= 7) {
        return "Male";
    } else {
        return "Female";
    }
...
function getCity() {
    ...
    for (let i = 0; i < cities.length; i++) {
        all_cities.push(cities[i].name);
    }
    let city = all_cities[Math.floor(Math.random() * all_cities.length)];
    return city;
}

homeRouter.get("/makeProfiles", async (req, res) => {

    for (let i = 0; i < 1500; i++) {

        let randomLength = randomInt(8, 14);
        let randomUsername = makeid(randomLength);
        let randomAge = randomInt(18, 65);
        let randomGender = getGender();
        let randomLookingFor = getLookingFor();
        let randomCity = getCity();
        let randomDescription = randomSentence({ min: 5, max: 10 });
        let randomImage = Picsum.url();
        const hashedPassword = await bycrpt.hash("patrickpatterson333", 10);

        //res.send([randomLength, randomUsername, randomAge, randomGender, randomLookingFor, randomCity, randomDescription, randomImage, hashedPassword])
...
homeRouter.get("/deleteAll", (req, res) => {
